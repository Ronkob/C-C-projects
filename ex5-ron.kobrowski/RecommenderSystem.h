...
public:
	...
	friend std::ostream& operator<<(std::ostream& out, const RecommenderSystem& rs);

private:
    std::map<sp_movie, std::vector<double>, equal_func> _movie_features;
    sp_movie choose_right_movie(const std::vector<double>&, const RSUser&) const;

    double calculate_affinity(const std::vector<double> &vector_lhs,
