 ...
 *@Updates: [VERSION 1.6]
 *
 * 0. Added a test that check special key types such as HashMap<int *, int>
 *    and HashMap<key_struct, int> (key_struct declared at line 39, notice
 *    that I wrote a method that specialize std::hash to work with the new
 *    key_struct object I created).
 *
...
struct key_struct {
    ...
    key_struct(string xp, int yp) : x(xp),y(yp) {}
};
...
void test_iterator_begin_end ()
{
  START_TEST;
  HashMap<int, int> h1;
  assert(typeid (h1.begin ()) == typeid (HashMap<int, int>::const_iterator));
...
/**
 * @tests:
 * 0. Test if operators work as expected
 */
...
/**
 * @tests:
 * 0. erase() throws InvalidKey when given non-existing key
...
void test_invalid_key_exception ()
{
  ...
  {
    assert(false); // InvalidKey should be derived from invalid argument
  }
}
